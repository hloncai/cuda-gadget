...
int force_treebuild_single(int npart)
{
  ...
  for(i = 0; i < npart; i++)
	...
	/* Tolerance checking */
	/*	const double CELLTOL = 1.e-5;
	      if(P[i].Pos[0] > (Nodes[th].center[0] - CELLTOL) && P[i].Pos[0] < (Nodes[th].center[0] + CELLTOL))
		printf("Particle %d positioned within tree cell boundary tolerance.\n",i);
	      if(P[i].Pos[1] > (Nodes[th].center[1] - CELLTOL) && P[i].Pos[1] < (Nodes[th].center[1] + CELLTOL))
		printf("Particle %d positioned within tree cell boundary tolerance.\n",i);
	      if(P[i].Pos[2] > (Nodes[th].center[2] - CELLTOL) && P[i].Pos[2] < (Nodes[th].center[2] + CELLTOL))
		printf("Particle %d positioned within tree cell boundary tolerance.\n",i);
	*/
	      ...
	      if(nfreep->len < 1.0e-3 * epsilon)
		{
		   ...
		   * length-scale anyway.
		   */
		  printf("TREERND triggered with length %f\n",nfreep->len);
...
void force_treeallocate(int maxnodes, int maxpart)
{
  ...
  if(first_flag == 0)
    ...
    ASSERT_GX(NTAB==1000);
    UpdateShortrangeTableData_gx(NTAB,shortrange_table);
}
