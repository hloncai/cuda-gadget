...
int main(int argc, char **argv)
{
  ...
  /* Create localhost communicator and get local process rank */
  char hostname[256];
  int localhost_rank;
  gethostname(hostname, sizeof(hostname));
  
  MPI_Comm MPI_LOCALHOST;
  MPI_Comm_split(MPI_COMM_WORLD, stringhash(hostname), ThisTask, &MPI_LOCALHOST);
  MPI_Comm_rank(MPI_LOCALHOST, &localhost_rank);
  
  printf("Host %s has local rank %d\n", hostname, localhost_rank);

  /* Wait for debugger to attach to Task 0 (if debugging is enabled) */
/*  if (ThisTask == 0)
  {
  	int i = 0;
    char hostname[256];
  ...
  /* Resume normal program execution for all processes now */
  
  /* BDW: Serialize the device initialization on a given host -- otherwise the CUDA 5 driver bugs out... :( */
  int i;
  for (i=0; i<NTask; i++)
  {
  	MPI_Barrier(MPI_LOCALHOST);	// synchronize tasks on this host
  	if (i==ThisTask)
  	{
  		Initialize_gx(argc,argv,ThisTask,NTask,localhost_rank); // GX mod, use gfx card N on a multicard GPU
  	}
  	MPI_Barrier(MPI_LOCALHOST);
  }
  
  TimerReset(ThisTask);                    // GX mod, reset timers
  TimerBeg(0);
