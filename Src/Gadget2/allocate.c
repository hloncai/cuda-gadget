...
void allocate_memory(void)
{
  ...
  if(All.MaxPart > 0)
      ...
      if(!(P = malloc(bytes = All.MaxPart * sizeof(struct particle_data))))
	{
      ...
      if(ThisTask == 0)
	printf("\nAllocated %g MByte for particle storage. %d\n\n", bytes_tot / (1024.0 * 1024.0), (int)sizeof(struct particle_data));
    }
  ...
  if(All.MaxPartSph > 0)
	...
	{
	  printf("failed to allocate memory for `SphP' (%g MB) %d.\n", bytes / (1024.0 * 1024.0), (int)sizeof(struct sph_particle_data));
	  endrun(1);
	}
      bytes_tot += bytes;

      if(ThisTask == 0)
	printf("Allocated %g MByte for storage of SPH data. %d\n\n", bytes_tot / (1024.0 * 1024.0), (int)sizeof(struct sph_particle_data));
    }
}
