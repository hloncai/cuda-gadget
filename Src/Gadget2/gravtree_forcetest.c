  ...
  while(ntotleft > 0)
	...
	if(P[i].Ti_endstep < 0)
	  {
	    ...
	    /* NO Let's disable direct force calculation and just output the tree forces */
      ...
      for(level = 1; level < (1 << PTask); level++)
	{
	  ...
	  for(j = 0; j < nbuffer[ThisTask]; j++)
	    {
		/* Disable direct force calc for now */
      ...
      if(nthis == ThisTask)
	{
	  ...
	  for(i = 0; i < NumPart; i++)
	    if(P[i].Ti_endstep < 0)
	      {
#ifndef PMGRID
		fprintf(FdForceTest, "%d %20.10f %20.10f %20.10f %20.10f %20.10f %20.10f %20.10f %20.10f %20.10f %20.10f\n",
			i, All.Time, All.Time - TimeOfLastTreeConstruction,
			P[i].Pos[0], P[i].Pos[1], P[i].Pos[2],
			P[i].GravAccelDirect[0], P[i].GravAccelDirect[1], P[i].GravAccelDirect[2],
			P[i].GravAccel[0], P[i].GravAccel[1], P[i].GravAccel[2]);
#else
		fprintf(FdForceTest, "%d %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
			i, P[i].Type, All.Time, All.Time - TimeOfLastTreeConstruction,
			P[i].Pos[0], P[i].Pos[1], P[i].Pos[2],
			P[i].GravAccelDirect[0], P[i].GravAccelDirect[1], P[i].GravAccelDirect[2],
			P[i].GravAccel[0], P[i].GravAccel[1], P[i].GravAccel[2],
