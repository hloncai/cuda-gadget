...
void force_treeevaluate_shortrange_cuda_gx_kernel()
{
	...
	FLOAT_INTERNAL_GX r2, dx, dy, dz, mass, h, h_inv, h3_inv;
	FLOAT_INTERNAL_GX pos_x, pos_y, pos_z, aold;
	FLOAT_INTERNAL_GX rcut, asmth;

	#if defined(UNEQUALSOFTENINGS) && !defined(ADAPTIVE_GRAVSOFT_FORGAS)
		int maxsofttype;
	#endif
	#ifdef ADAPTIVE_GRAVSOFT_FORGAS
		FLOAT_INTERNAL_GX soft = 0;
	#endif
	#ifdef PERIODIC
		const FLOAT_INTERNAL_GX boxsize = g_const_parameters.BoxSize;
		const FLOAT_INTERNAL_GX boxhalf = 0.5 * g_const_parameters.BoxSize;
	#endif
	...
	for(target_x=tid.x;target_x<tid.y;target_x+=tid.z)
	{
		...
		// 2a:
		FLOAT_INTERNAL_GX acc_x = 0;
		FLOAT_INTERNAL_GX acc_y = 0;
		FLOAT_INTERNAL_GX acc_z = 0;
		int ninteractions = 0;
		...
		const FLOAT_INTERNAL_GX rcut2 = rcut * rcut;
		const FLOAT_INTERNAL_GX asmthfac = 0.5 / asmth * (g_const_state.sz_shortrange_table / 3.0);
		...
		while(no >= 0)
		{
			...
			if(no < g_const_parameters.MaxPart) {
				...
				r2 = dx * dx + dy * dy + dz * dz;
					...
					#ifdef ADAPTIVE_GRAVSOFT_FORGAS
						const FLOAT_INTERNAL_GX Hsml=GetHsml(no); // SphP[no].Hsml
						if(ptype == 0) h = soft;
				...
				// 3d1:
				float NODE_gx_len;
				float NODE_gx_center0;
				float NODE_gx_center1;
				float NODE_gx_center2;
				...
				{
					int xno = no - g_const_state.MaxPart;
					ASSERT_DEVICE_GX( xno<g_const_state.sz_Nodes_base );
					xno *= 3;

					int4 i4;
					#ifdef CUDA_GX_USE_TEXTURES
						i4=tex1Dfetch(tex_Nodes_base,xno);
					#else
						i4=((int4*)g_const_state.Nodes_base)[xno];
					#endif

					NODE_gx_len    = *(float*)(&i4.x);
					NODE_gx_center0= *(float*)(&i4.y);
					NODE_gx_center1= *(float*)(&i4.z);
					NODE_gx_center2= *(float*)(&i4.w);
					...
					NODE_gx_s0     = *(float*)(&i4.x);
					NODE_gx_s1     = *(float*)(&i4.y);
					NODE_gx_s2     = *(float*)(&i4.z);
					NODE_gx_mass   = *(float*)(&i4.w);
					...
					NODE_gx_bitflags = *(int*)(&i4.x);
					NODE_gx_sibling  = *(int*)(&i4.y);
					NODE_gx_nextnode = *(int*)(&i4.z);
					//NODE_gx_father   = *(int*)((&i4.w);
				}
				...
				*/
				...
				if(r2 > rcut2) {
					...
					const FLOAT_INTERNAL_GX eff_dist = rcut + 0.5 * NODE_gx_len;
					FLOAT_INTERNAL_GX dist;
			...
			{ //  local scope for variables r, fac and tabindex
				...
				if(r >= h) fac = mass / (r2 * r);
						...
						h3_inv = h_inv * h_inv * h_inv;
					#endif
					const FLOAT_INTERNAL_GX u = r * h_inv;
